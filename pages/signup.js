...
const Signup = () => {
  ...
  const signupHandler = async (e) => {
    ...
    const { dataDb, errorDb } = await supabase
      .from("users")
      .insert([{ username: signupForm.username, email: signupForm.email }]);

    if (errorDb) {
      console.log("Error from database:", errorDb);
      return;
    }
    ...
    <main className="w-full flex">
      <div className="relative flex-1 hidden items-center justify-center h-screen bg-gray-900 lg:flex">
        <div className="relative z-10 w-full max-w-md">
          ...
          <div className=" mt-16 space-y-3">
            <h3 className="text-white text-3xl font-bold">
              Start tracking your porfolio.
            </h3>
            <p className="text-gray-300">
              Get access to all features including dashboard you can analyze
              what is yours.
            </p>
          </div>
        </div>
        <div
          ...
          style={{
            background:
              "linear-gradient(152.92deg, rgba(252, 204, 132, 0.2) 4.54%, rgba(249, 219, 121, 0.26) 34.2%, rgba(252, 204, 132, 0.1) 77.55%)",
            filter: "blur(118px)",
          }}
        ></div>
      </div>
      <div className="flex-1 flex items-center justify-center h-screen">
        <div className="w-full max-w-md space-y-8 px-4 bg-transparent text-gray-600 sm:px-0">
          ...
          <div className="relative">
            ...
            <p className="inline-block w-fit text-sm bg-[#f3f4f6] px-2 absolute -top-3 inset-x-0 mx-auto">
              Or
            </p>
          </div>
